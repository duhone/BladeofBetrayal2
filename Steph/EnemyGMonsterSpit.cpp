...
void EnemyGMonsterSpit::processAttack()
{
	if(!ipFlags.S_ATTACK && !ipFlags.S_DAMAGED && !ipFlags.S_DEAD)
	{
		...
		SetAnimation(ENEMY_SPIT_MONSTER_ATTACK, 0, true, true, 10, true);
			...
			{
				//temp = new ProjectileGMonsterSpit(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet + 5), static_cast<int>(yLoc));
				...
				temp = new ProjectileAGMSpit(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet + 20), static_cast<int>(yLoc+15),*projectile_list,m_player->GetXLoc());
