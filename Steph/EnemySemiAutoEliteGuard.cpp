...
void EnemySemiAutoEliteGuard::processAttack()
{
	if(!ipFlags.S_ATTACK)
	{
		...
		case 1:
			...
			{
				temp = new ProjectileEliteGuardRifleBullet(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet+20), static_cast<int>(yLoc- 6),.25f);
				
				(*projectile_list).push_back(temp);

				temp = new ProjectileEliteGuardRifleBullet(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet+20), static_cast<int>(yLoc- 6),.40f);
			
				(*projectile_list).push_back(temp);

				temp = new ProjectileEliteGuardRifleBullet(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet+20), static_cast<int>(yLoc- 6),.55f);
