...
void EnemyMrThomas::processAttack()
{
	...
	if(!ipFlags.S_ATTACK)
	{
		...
		case 1: /* Projectile Attack */
			{
//				sound_processor->PlaySound(game_sounds->phase00, ipFlags.S_ACTIVE);
				...
				{
					temp = new ProjectileMrThomasPhase(static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc), maxWalkSpeed*4, &ipFlags, &xLoc, &yLoc);
		...
		case 3:
			{
				...
				{		
				 ...
				 if(ipFlags.S_DIRECTION)	
				 {
				 temp = new ProjectileMrThomasPhase(static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc), maxWalkSpeed*phaseSpeedMod, &ipFlags, &xLoc, &yLoc,m_leftBound,m_rightBound);
		...
		case 1:
			if(!player_spr->IsAnimating())
			{
				if(!m_threw)
				{
					...
					if(ipFlags.S_DIRECTION)	
					{
						temp = new ProjectileMrTKnife(ipFlags.S_DIRECTION,static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc), false);
					}
					else
					{
						temp = new ProjectileMrTKnife(ipFlags.S_DIRECTION,static_cast<int>(xLoc - nxOffSet), static_cast<int>(yLoc), false);						
		...
		case 2:
			if(!player_spr->IsAnimating())
			{
				if(!m_threw)
				{
					...
					if(ipFlags.S_DIRECTION)	
					{
						temp = new ProjectileMrTKnife(ipFlags.S_DIRECTION,static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc), true);
					}
					else
					{
						temp = new ProjectileMrTKnife(ipFlags.S_DIRECTION,static_cast<int>(xLoc - nxOffSet), static_cast<int>(yLoc), true);						
