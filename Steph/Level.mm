...
{
#if !defined HPTMIPS
	tile_background_back = graphics_engine->CreateTileBackground();
#endif

	tile_background_front = graphics_engine->CreateTileBackground();
	level_data = 0;
	object_array = 0;
	player = 0;
	projectile_list = 0;
...
{	
	...
	char tempc[100];
#if !defined HPTMIPS
	sprintf(tempc,("level%db"),level_number);
	NSString * path1 = [[NSBundle mainBundle] pathForResource: [[NSString alloc] initWithCString: tempc] ofType: @"htf"];
	const char *levelfile1 = [path1 cStringUsingEncoding:1];
	tile_background_back->LoadTileSet(const_cast<char*>(levelfile1));

#endif
	sprintf(tempc,("level%df"),level_number);
	path1 = [[NSBundle mainBundle] pathForResource: [[NSString alloc] initWithCString: tempc] ofType: @"htf"];
	levelfile1 = [path1 cStringUsingEncoding:1];
	tile_background_front->LoadTileSet(const_cast<char*>(levelfile1));
#if !defined HPTMIPS
...
void Level::SetLevelPosition(int x, int y)
{

	level_position.p1.x = x;
	level_position.p1.y = y;
	level_position.p2.x = (x>>2);
	level_position.p2.y = (y>>2);
	//char tempc[100];
	//sprintf(tempc,"%d %d",level_position.p2.x,level_position.p2.y);
	//OutputDebugString(tempc);
	tile_background_front->SetPositionAbsalute(level_position.p1.x,level_position.p1.y);
	tile_background_back->SetPositionAbsalute(level_position.p2.x,level_position.p2.y);
...
void Level::LoadObjects(bool _enemiesOnly)
{
	...
	for(int ycount = 0;ycount < level_y_size;ycount++)
	{
		for(int xcount = 0;xcount < level_x_size;xcount++)
		{
			...
			Object* newObject = g_objectFactory::Instance().Create(temp);
			
			if(newObject)
			{
				newObject->SetWorldPosition((xcount<<4) + 8,(ycount<<4) + 8);
				object_list.push_back(newObject);
				*(object_array+count) = newObject;
				count++;
				continue;
			}
