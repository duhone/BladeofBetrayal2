...
HPTRect &EnemyBlueNinja::GetWeaponWorldLoc()
{
	...
	{
		SetWeaponDamage(2);
		SetRectangle(rectWeaponWorldLoc, 25, 4, nxOffSet, 2);//, ipFlags.S_DIRECTION);
	}
...
//			OutputDebugString("Blue: Ninja Star - Throw");
			...
			{
				temp = new ProjectileNinjaStar(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc - 15));
			}
			else
			{
				temp = new ProjectileNinjaStar(ipFlags.S_DIRECTION, static_cast<int>(xLoc - nxOffSet), static_cast<int>(yLoc - 15));
