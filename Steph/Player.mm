...
Player::Player():xWorldOffset(0),yWorldOffset(0),steph_top(NULL)
{
	number_grenades = 0;
	memset(weapons,0,7);
	weapons[0] = true;
...
void Player::doDamage(int nDamageAmount, bool sDirection,int damage_type)
{
	...
	if(!ipFlags.S_DAMAGED && !S_BARRIER_SHIELD)
	{
		if(!ipFlags.S_INVULNERABLE)
		{
			...
			int dmgAmt =  static_cast<int>(nDamageAmount);
...
void Player::CalcWorldOffset()
{
	...
	if(xLoc < 240)
		xWorldOffset = static_cast<int>(-(240-xLoc+xTransformOffset+xQuakeAttackModifier));
	else if (xLoc > (hptRectTemp.p2.x))
		xWorldOffset = static_cast<int>(-((hptRectTemp.p2.x)-xLoc+xTransformOffset+xQuakeAttackModifier));
	else 
		xWorldOffset = 0;

	if(yLoc < 160)
		yWorldOffset = static_cast<int>(-(160-yLoc+yTransformOffset+yQuakeAttackModifier));
	else if(yLoc > (hptRectTemp.p2.y)) 
		yWorldOffset = static_cast<int>(-(hptRectTemp.p2.y-yLoc+yTransformOffset+yQuakeAttackModifier));
	else
		yWorldOffset = 0;
...
void Player::processAttack()
{
	...
	if(!ipFlags.S_ATTACK)
	{
		if(ipFlags.S_CROUCH || ipFlags.S_CRAWL)
		{
			soundPlayer->PlaySound(swing00);
			SetAnimation(PLAYER_ANIM_ATTACK_CROUCH, 0, true, true, 25, true);
			SetWeaponDamage(static_cast<int>(basic_weapon_damage_amount));
...
//		}
		else if(!ipFlags.S_ON_LADDER && !ipFlags.S_FALLING)// if(ipFlags.S_ON_GROUND)
		{
			if(steph)
			{
				...
				GetVelocityVector(&xVelocity, &yVelocity);
				
				//(*projectile_list).push_back(equippedWeapon->Attack());
				...
				case 1:
					{
						if(steph)
						{
							...
							steph_top->AutoStopAnimate();
								...
								if(ipFlags.S_DIRECTION)
								{
									if(game_class->AnalogX() > (-3*game_class->AnalogY()))  temp = new ProjectileStephPistol(0, static_cast<int>(xLoc+7)+nxOffSet, static_cast<int>(yLoc-8),xveladd,yveladd,this,nxOffSet+7,-8);
									else if((-game_class->AnalogY()) > (3*game_class->AnalogX())) temp = new ProjectileStephPistol(90, static_cast<int>(xLoc -18)+nxOffSet, static_cast<int>(yLoc -24 ),xveladd,yveladd,this,nxOffSet-18,-24);
									else temp = new ProjectileStephPistol(45, static_cast<int>(xLoc -5)+nxOffSet, static_cast<int>(yLoc -17 ),xveladd,/*yveladd*/0,this,nxOffSet-5,-17);
								...
								{
									xveladd = -xveladd;
									if((-game_class->AnalogX()) > (-3*game_class->AnalogY()))  temp = new ProjectileStephPistol(180, static_cast<int>(xLoc - 7)-nxOffSet, static_cast<int>(yLoc -8 ),xveladd,yveladd,this,nxOffSet-7,-8);
									else if((-game_class->AnalogY()) > (-3*game_class->AnalogX())) temp = new ProjectileStephPistol(90, static_cast<int>(xLoc +18)-nxOffSet, static_cast<int>(yLoc -24 ),xveladd,yveladd,this,18-nxOffSet,-24);
									else temp = new ProjectileStephPistol(135, static_cast<int>(xLoc +5)-nxOffSet, static_cast<int>(yLoc -17 ),xveladd,/*yveladd*/0,this,5-nxOffSet,-17);
									//temp = new ProjectileStephPistol(ipFlags.S_DIRECTION, static_cast<int>(xLoc - 40), static_cast<int>(yLoc -5 ));
									temp->SetWeaponDamage(4);
								}
								temp->SetSource(true);
						...
						if(steph)
						{
							...
							if(ipFlags.S_DIRECTION)
							{
								...
								/*if(game_class->twCurKeyStates.twkInputNavX > (-3*game_class->twCurKeyStates.twkInputNavY))
								{
									...
									xoff = nxOffSet+10;
									yoff = -2;
									//temp = new ProjectileStephPistol(0, static_cast<int>(xLoc+7)+nxOffSet, static_cast<int>(yLoc-8),xveladd,yveladd,this,nxOffSet+7,-8);
								}
								else if((-game_class->twCurKeyStates.twkInputNavY) > (3*game_class->twCurKeyStates.twkInputNavX))
								{
									...
									xoff = nxOffSet-10;
									yoff = -26;
									//temp = new ProjectileStephPistol(90, static_cast<int>(xLoc -18)+nxOffSet, static_cast<int>(yLoc -24 ),xveladd,yveladd,this,nxOffSet-18,-24);
								}
								else
								{
									...
									yoff = -17;
									angle = 45;
									//temp = new ProjectileStephPistol(45, static_cast<int>(xLoc -5)+nxOffSet, static_cast<int>(yLoc -17 ),xveladd,yveladd,this,nxOffSet-5,-17);
								}*/
//									
							...
							else
							{
								...
								/*if((-game_class->twCurKeyStates.twkInputNavX) > (-3*game_class->twCurKeyStates.twkInputNavY)) 
								{
									...
									xoff = nxOffSet-24;
									yoff = -2;
									//temp = new ProjectileStephPistol(180, static_cast<int>(xLoc - 7)-nxOffSet, static_cast<int>(yLoc -8 ),xveladd,yveladd,this,nxOffSet-7,-8);
								}
								else if((-game_class->twCurKeyStates.twkInputNavY) > (-3*game_class->twCurKeyStates.twkInputNavX))
								{
									angle = 90;
									xoff = 14-nxOffSet;
									yoff = -26;
									//temp = new ProjectileStephPistol(90, static_cast<int>(xLoc +18)-nxOffSet, static_cast<int>(yLoc -24 ),xveladd,yveladd,this,18-nxOffSet,-24);
								}
								else 
								{
									angle = 135;
									xoff = 2-nxOffSet;
									yoff = -17;
									//temp = new ProjectileStephPistol(135, static_cast<int>(xLoc +5)-nxOffSet, static_cast<int>(yLoc -17 ),xveladd,yveladd,this,5-nxOffSet,-17);
								}*/
...
void Player::Render()
{
	player_spr->SetImage(CR::AssetList::Steph_lower);
	if(S_JACOB_INVIS)
		return;
	...
	if(ipFlags.S_RENDER)
	{
		...
		if(((player_spr->GetFrameSet() >= 0) && (player_spr->GetFrameSet() <= 6)) || (player_spr->GetFrameSet() == 24))
		{
			...
			if (player_spr->GetFrameSet() == PLAYER_ANIM_CROUCH)
			{
				xPos = nxOffSet + xWorldOffset + xTransformOffset + xQuakeAttackModifier;
				yPos = 13 + yWorldOffset + yTransformOffset + yQuakeAttackModifier;
			}
			else
			{
				xPos = nxOffSet + xWorldOffset + xTransformOffset + xQuakeAttackModifier;
				yPos = yWorldOffset + yTransformOffset + yQuakeAttackModifier;
			}
			
			equippedWeapon->Update(ipFlags.S_DIRECTION, game_class->AnalogX(), game_class->AnalogY(), xPos, yPos);
...
void Player::UpdateLevel()
{
	...
	xLevelPos = static_cast<int>(240-xLoc+xTransformOffset);
	yLevelPos = static_cast<int>(160-yLoc+yTransformOffset);
	...
	curLevel->SetLevelPosition(static_cast<int>(xLevelPos+xQuakeAttackModifier), static_cast<int>(yLevelPos+yQuakeAttackModifier));
}
...
void Player::PrevAttack()
{
	...
	{
		ipFlags.nCurrAttack--;		
		if(ipFlags.nCurrAttack < 0) ipFlags.nCurrAttack = 5;
...
void Player::SetAttack(int nAttack)
{
	...
	if(weapons[ipFlags.nCurrAttack]) return;
...
void Player::SetGrenades(int arg)
{
	number_grenades = arg;
...
void Player::EquipWeapon(Weapon *weapon)
{
	...
	equippedWeapon->SetPlayerSprite(steph_top);
}
