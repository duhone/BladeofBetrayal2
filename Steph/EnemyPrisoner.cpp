...
HPTRect &EnemyPrisoner::GetWeaponWorldLoc()
{
	...
	{
		SetWeaponDamage(4);
		SetRectangle(rectWeaponWorldLoc, 15, 4, nxOffSet, 2);//, ipFlags.S_DIRECTION);
	}
...
/*	if(player_spr->GetFrame() == 5)
	...
	{
		rectWeaponWorldLoc.p1.x = static_cast<int>(xLoc + nxOffSet);
		rectWeaponWorldLoc.p2.x = static_cast<int>(xLoc + 20 + nxOffSet);
		rectWeaponWorldLoc.p1.y = static_cast<int>(yLoc - 2);
		rectWeaponWorldLoc.p2.y = static_cast<int>(yLoc + 2);
	}
	else
	{
		rectWeaponWorldLoc.p1.x = static_cast<int>(xLoc - 20 - nxOffSet);
		rectWeaponWorldLoc.p2.x = static_cast<int>(xLoc - nxOffSet);
		rectWeaponWorldLoc.p1.y = static_cast<int>(yLoc - 2);
		rectWeaponWorldLoc.p2.y = static_cast<int>(yLoc + 2);
	}*/
