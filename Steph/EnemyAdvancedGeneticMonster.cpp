...
{
	ForEach(m_sprites,mem_fun(&CR::Graphics::Sprite::Release));
	m_sprites.clear();
	player_spr = NULL;
	delete healthBar;
	bossIcon->Release();
...
//				sound_processor->PlaySound(game_sounds->swoosh);
			...
			case 1: /* CLAW ATTACK */
			{
				...
				{
					temp = new ProjectileAgmBone(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc - 15));
...
{	
	...
	if(m_superJumping)
	{
		...
		else
		{
			...
			yLoc = m_jumpStart.y-pow(sin(percent*3.1459f),m_jumpXP)*m_jumpHeight;
		}
		return;
	}
...
//	BelowFeetLadderCheck();
	...
	if(ipFlags.S_ATTACK  )
	{
		...
		if(ipFlags.nCurrAttack == 2 && !m_firedSpikes && player_spr->GetFrame() >= 8)
		{
			...
			Projectile *temp;
			temp = new ProjectileAgmBone(false, static_cast<int>(xLoc-20 + nxOffSet), static_cast<int>(yLoc - 15),true);
			(*projectile_list).push_back(temp);
			temp = new ProjectileAgmBone(false, static_cast<int>(xLoc-10 + nxOffSet), static_cast<int>(yLoc - 15),true);
			(*projectile_list).push_back(temp);
			temp = new ProjectileAgmBone(false, static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc - 15),true);
			(*projectile_list).push_back(temp);
			temp = new ProjectileAgmBone(false, static_cast<int>(xLoc+10 + nxOffSet), static_cast<int>(yLoc - 15),true);
			(*projectile_list).push_back(temp);
			temp = new ProjectileAgmBone(false, static_cast<int>(xLoc+20 + nxOffSet), static_cast<int>(yLoc - 15),true);
		...
		if(ipFlags.nCurrAttack == 3)
		{
			...
			{
				Projectile *temp;
				temp = new ProjectileAgmBone(true, static_cast<int>(xLoc+m_dropOffset + nxOffSet), static_cast<int>(-10-yViewLoc),true);
				//cout << "spawned at" << xLoc-m_dropOffset + nxOffSet << endl;
				(*projectile_list).push_back(temp);
				m_dropOffset -= 70;
				m_droppedSpikes++;
				m_spikeDropTime = 0;
		...
		{			
			...
			{				
				...
				SetAnimation(0, 0, true, true, 15, true);
				Projectile *temp;
				temp = new ProjectileAGMSpit(false, static_cast<int>(xLoc-20 + nxOffSet), static_cast<int>(yLoc - 15),*projectile_list,m_player->GetXLoc());
				(*projectile_list).push_back(temp);
				++m_numSpits;
