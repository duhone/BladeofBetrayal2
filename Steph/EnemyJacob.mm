...
void EnemyJacob::Render()
{
	...
	//ForEach(m_rain, mem_fun(&Sprite::Render));
}
		...
		switch (ipFlags.nCurrAttack)
		{
			...
			case 2:				
				if(!player_spr->IsAnimating())
				{
					...
					if(ipFlags.S_DIRECTION)
					{
						temp = new ProjectileGroundWave(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet + 15), static_cast<int>(yLoc + 36));
...
//	SetRectangle(rectWorldLoc, 30, 60, -nxOffSet, -15);
}
