...
//		if((music != 0) && (game->mute_music != true)) music->HPTPlaySound();
	...
	if(arg >= 1)
	{
		...
		skipMovieButton->SetSpriteAndBounds(210, 280, AssetList::button_skip);
		skipMovieButton->SetSoundOn(true);
		skipMovieButton->Disabled(false);
		
		prevMovieButton->SetSpriteAndBounds(5, 280, AssetList::button_previous);
		prevMovieButton->SetSoundOn(true);
		prevMovieButton->Disabled(true);
		
		nextMovieButton->SetSpriteAndBounds(420, 280, AssetList::button_next);
		nextMovieButton->SetSoundOn(true);
...
//death slowly fades away as he exhales, liberated from vengeance...
		...
		default:
			...
			{
				skipMovieButton->SetPosition(395, 265);
				/*
				 ...
				 */
				 ...
				 (*graphics_engine) << game_class->bankGothic << game_class->GetAchievementsCount() << "/27";
			}
...
void Theater::CreateSceneLite()
{
	...
	sprintf(temp.top,"");
	sprintf(temp.bottom,"");
	sprintf(temp.dialog,"Buy Bob now, cheap bastards!");
	scenes.push_back(temp);
