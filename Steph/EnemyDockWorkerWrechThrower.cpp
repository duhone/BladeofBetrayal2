...
void EnemyDockWorkerWrechThrower::processAttack()
{
	if(!ipFlags.S_ATTACK && !ipFlags.S_DAMAGED && !ipFlags.S_DEAD)
	{
		...
		case 1:
			...
			{
				temp = new ProjectileWrench(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc - 15), 75, -100,0,200);
		...
		case 2:
			...
			if(ipFlags.S_DIRECTION)
			{
				temp = new ProjectileWrench(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet), static_cast<int>(yLoc - 15), 130, 0,0,0);
