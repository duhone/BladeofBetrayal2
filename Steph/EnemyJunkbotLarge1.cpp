...
void EnemyJunkbotLarge::processAttack()
{
	if(!ipFlags.S_ATTACK)
	{
		...
		{
			temp = new ProjectileJunkbotLarge(ipFlags.S_DIRECTION, static_cast<int>(xLoc + nxOffSet + 3), static_cast<int>(yLoc - 13));
...
//	BelowFeetLadderCheck();
	...
	SetRectangle(rectWorldLoc, 40, 80, -nxOffSet, -50);//-55);
}